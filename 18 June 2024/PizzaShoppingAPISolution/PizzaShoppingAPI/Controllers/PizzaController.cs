...
namespace PizzaShoppingAPI.Controllers
{
    ...
    public class PizzaController : ControllerBase
    {
        ...
        [ProducesResponseType(typeof(ErrorModel), StatusCodes.Status404NotFound)]
        [ProducesErrorResponseType(typeof(ErrorModel))]
        public async Task<IActionResult> UpdatePizza([FromBody] PizzaDTO pizzaDto)
        {
            try
            {
                await _pizzaService.UpdatePizzaAsync(pizzaDto.Id, pizzaDto);
                return NoContent();
            }
            catch (NoSuchPizzaException pnfe)
            {
                return NotFound(new ErrorModel(404, pnfe.Message));
            }
        }
    }
}
